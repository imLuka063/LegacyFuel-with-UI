...
local LukaTankencallback = false
local LukaKanistercallback = false
...
AddEventHandler('fuel:refuelFromPump', function(pumpObject, ped, vehicle)
	...
	while isFueling do
		...
		if IsControlJustReleased(0, 38) or DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) or (isNearPump and GetEntityHealth(pumpObject) <= 0) then
			LukaTankencallback = false
			ESX.ShowNotification("Tanken beendet")
			isFueling = false
		end
	end
                    ...
                    if not DoesEntityExist(GetPedInVehicleSeat(vehicle, -1)) then
                        ...
                        if GetVehicleFuelLevel(vehicle) < 95 and canFuel then
                            if currentCash > 0 then
                                ESX.ShowHelpNotification(Config.Strings.EToRefuel)

                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent('showTankenUI')
                                end 

                                if LukaTankencallback == true then
                                    isFueling = true
...
RegisterNetEvent('showTankenUI')
AddEventHandler('showTankenUI', function()
    uiVisible = true
    SetNuiFocus(true, true) -- Enable NUI focus to show the mouse cursor and allow UI interaction
    FreezeEntityPosition(PlayerPedId(), true) -- Freeze the player
...
RegisterNetEvent('hideTankenUI')
AddEventHandler('hideTankenUI', function()
    uiVisible = false
    SetNuiFocus(false, false) -- Disable NUI focus
    FreezeEntityPosition(PlayerPedId(), false) -- Unfreeze the player
...
RegisterNUICallback('imlukaaufdie1', function(data, cb)
    local ped = PlayerPedId()
    local playerCoords = GetEntityCoords(ped)
    local vehicle = GetClosestVehicle(playerCoords.x, playerCoords.y, playerCoords.z, 5.0, 0, 71) -- 5 units radius

    if DoesEntityExist(vehicle) and GetDistanceBetweenCoords(playerCoords, GetEntityCoords(vehicle), true) < 5.0 then
        LukaTankencallback = true
        ESX.ShowNotification("Tanken gestartet")
		TriggerEvent('hideTankenUI') -- Hide the UI after selection
    else
...
RegisterNUICallback('cancel', function(data, cb)
    TriggerEvent('hideTankenUI')
    cb('ok')
end)

RegisterNUICallback('hideTankenUI', function(data, cb)
    TriggerEvent('hideTankenUI')
    cb('ok')
end)